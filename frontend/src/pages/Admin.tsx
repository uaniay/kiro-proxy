...
export default function Admin() {
  ...
  useEffect(() => { loadUsers(); loadPool(); loadUsage(); }, []);
  ...
  const deletePool = async (id: string) => { if (!confirm('Delete?')) return; await api.deletePool(id); loadPool(); };
  const togglePool = async (id: string, enabled: boolean) => { await api.togglePool(id, !enabled); loadPool(); };
