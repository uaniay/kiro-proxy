...
use crate::converters::kiro_to_responses;
use crate::converters::responses_to_kiro;
...
pub(crate) async fn responses_handler(
    ...
    let request: ResponsesApiRequest = serde_json::from_slice(&body_bytes)
    ...
    let kiro_result = responses_to_kiro::build_kiro_payload(&request, &conversation_id, &profile_arn, &config)
    ...
    if request.stream {
        ...
        let sse_stream = kiro_to_responses::stream_kiro_to_responses(
            response,
            &request.model,
        ...
        let body = kiro_to_responses::collect_responses_response(
            response,
            &request.model,
            config.first_token_timeout,
            input_tokens,
...
fn estimate_input_tokens(request: &ResponsesApiRequest) -> i32 {
    let mut total = 0;
    if let Some(ref instructions) = request.instructions {
        total += crate::tokenizer::count_tokens(instructions, false);
    }
    for item in &request.input {
        let text = serde_json::to_string(item).unwrap_or_default();
        total += crate::tokenizer::count_tokens(&text, false);
    }
    total
}
